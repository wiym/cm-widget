(function ($) {
    ...
    var index = 0;
    var config = {
        apiKey: "AIzaSyCbeGEMYr-LpdfjOGzgdc79NTema7u9D_Q",
        authDomain: "boussole-6a93d.firebaseapp.com",
        databaseURL: "https://boussole-6a93d.firebaseio.com",
        storageBucket: "boussole-6a93d.appspot.com",
        messagingSenderId: "273277063735"
    };
    firebase.initializeApp(config);

    var database = firebase.database().ref('feedbacks');
...
'</svg>'+
    ...
    function actionSendPopup() {
        ...
        if(note === '' && comment === '') {
          ...
          if ($('input[data-checkbox="feedback"]').is(':checked')) {
              // AT-PLAYGROUND
              webhook_url = 'https://hooks.slack.com/services/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';

          } else {
              // MICHEL TIXI
              webhook_url = 'https://hooks.slack.com/services/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';
          }
    ...
    function sendToFirebase(data) {
        ...
        database.once("value", function (snap) {
            ...
            data['browser'] = navigator.sayswho;
            ...
            if (window.location.href.indexOf('documents') !== -1) {
                ...
                tagToAdd = 'report';
                version = 'alpha';
            } else if (window.location.href.indexOf('ui-') !== -1) {
                tagToAdd = 'userinsight';
                application = 'userinsight';
                version = 'beta';
                channel = 'userinsight';
                applicationName = 'User Insight';
            } else {
                tagToAdd = 'explorer';
            ...
            // SEND TO SLACK
            if(userConfig.notificationSlack === true) {
              postMessageToSlack(userConfig.channelSlack);
            }
    ...
    function postMessageToSlack(channel) {
        var myJSONStr = '{ \
          ...
          "icon_url": "https://boussole-6a93d.firebaseapp.com/assets/favicon/android-chrome-512x512.png", \
          "attachments": [{\
              "fallback": "' + data['email'] + ' has sended a new feedback !",\
              "title": "New feedback",\
              "title_link": "http://' + config.authDomain + '/#/viewFeedback/' + data['eid'] + '",\
              "color": "#0CACF8",\
              ...
              "fields": [\
                    {\
                        "title": "Note",\
                        "value": "' + data['noteGlobale'] + '",\
                        "short": false\
                    },\
                ],\
          }]\
        }';
        $.post(webhook_url,myJSONStr).done(function() {
            console.log("Feedback slack notification success");
            note = 0;
            comment = '';
            console.log(note, comment);
        }).fail(function(error) {
            console.log( "Feedback slack notification error", error);
        });
    }
    ...
    function showNotification(type) {
        ...
        setTimeout(function () {
            $('[data-type-notification="' + type + '"]').addClass('fadeOutDown');
            $('[data-type-notification="' + type + '"]').removeClass('fadeInUp');
        }, 5000);
        setTimeout(function () {
            $('[data-type-notification="' + type + '"]').removeClass('fadeOutDown is-shown');
            $('[data-type-notification="' + type + '"]').removeClass('fadeInUp');
        }, 7000);
    }
